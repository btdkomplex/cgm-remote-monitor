...
def convert_nightscout(entries, start_time=None):
	...
	for entry in entries:
		...
		if entry["glucose"] and entry["is_sensor"]:
			...
			if entry["glucose"]:
				glucose = entry["glucoseInCurrentUnit"] if entry["glucoseInCurrentUnit"] and entry[
						"us_units"] else to_mgdl(entry["glucose"])
				dat.update({
					"eventType": "BG Check",
					"glucose": glucose,
					"glucoseType": "Finger",
...
def upload_nightscout(ns_format):
	...
	for ns in ns_format:
		if "sgv" in ns:
			sgv_upload.append(ns)
		else:
			treatment_upload.append(ns)

	upload = requests.post(NS_URL + 'api/v1/entries?api_secret=' + NS_SECRET, json=sgv_upload, headers={
		'Accept': 'application/json',
		'Content-Type': 'application/json',
		'api-secret': hashlib.sha1(NS_SECRET.encode()).hexdigest()
	})
	print("Nightscout SGV upload status:", upload.status_code, upload.text)

	upload = requests.post(NS_URL + 'api/v1/treatments?api_secret=' + NS_SECRET, json=treatment_upload, headers={
		'Accept': 'application/json',
		'Content-Type': 'application/json',
		'api-secret': hashlib.sha1(NS_SECRET.encode()).hexdigest()
	})
	print("Nightscout Treatment upload status:", upload.status_code, upload.text)
